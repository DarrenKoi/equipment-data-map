...
`metadata_evidence`, `interpretations`, `unresolved`. Samples preserve source path,
size, raw/UTC time, selection reason, bytes SHA and extract SHA. All references
...
Write evidence/extracts, base JSON and derived wiki/rag, then index listing every
...
source evidence references. RAG chunks carry family, source pattern, evidence
kind/SHA and claim type (observed fact or inference). Metadata-only evidence
...
| field meanings | Request one observed field at a time; plain text <= 512 characters or `UNKNOWN`; CLI keys the answer to that field |
