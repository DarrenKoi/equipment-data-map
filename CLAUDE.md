...
- **Approvals stay human.** `operator approve-plan`, `operator approve-result`, `operator unlock` appear in no skill and in no CLI `NEXT:` line. A skill invoking one is a scenario failure. `init` is the agent's: it passes the equipment id, host, port and roots the engineer gave it (chat or `engineer.toml [equipment]`) as flags; passwords and LLM keys never travel that way.
