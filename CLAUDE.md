...
`equipment-data-parser/` is the self-contained build-and-operate sequence for the company LLM: `index.md` is both the letter-execution contract and the workflow, then letters in order, tracking state in `office/progress.md`. `equipment-data-parser/spec.md` is a snapshot of the architecture doc; refresh it when the doc changes. `office/` (ledger, problem entries, `spike.py` workaround) belongs only to execution model folders — never create it during maintenance; read the summaries the user relays before rewriting a letter. The `office/` name is a repository path, not a signal for choosing a role.
...
- **Two stage axes.** The 6-step runtime pipeline runs inside one CLI invocation. The 5 rollout stages are what the skills map to: one stage skill each for stages 3–5, plus the `equipment-map-run` entry; stages 1–2 are build tests with no skill. Keep them apart.
...
- **Approvals stay human.** `operator approve-plan`, `operator approve-result`, `operator unlock` appear in no skill and in no CLI `NEXT:` line. A skill invoking one is a scenario failure. `init --equipment <file>` is the agent's: one TOML per equipment (host, user, password, roots) in a folder the engineer names in `engineer.toml [equipment]`; the CLI reads it and moves the password to the keystore, the agent only passes the path. A real rollout starts at stage 3: stages 1–2 are the build tests over fake FTP, proxy and LLM, and the engineer serves or types no fake values.
