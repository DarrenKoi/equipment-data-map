...
파일별 최대 바이트 budget이 무의미해진다. 대신 크기를 먼저 조회하고, budget을
넘는 파일은 내용을 받지 않고 메타데이터만 남긴다. 그 대가로 큰 파일의 내부
형식은 관측되지 않으며, 그 파일군은 `format: unknown`과 근거 `oversize`로
기록한다.
...
- 상위 디렉터리와 확장자
- 파일 크기 구간
- 사전 파일군마다 제한된 파일 수에서, 파일별 최대 바이트 이하인 파일만 통째로 받아 앞부분 바이트로 판정한 형식 signature

하나의 파일군은 “같은 생성 규칙과 내부 포맷을 가진 것으로 추정되는 파일 집합”이다. 그룹 규칙과 예외 파일을 함께 기록해 잘못 묶인 결과를 추적할 수 있게 한다. 형식 판정용 다운로드는 경로·확장자·크기로 만든 사전 파일군마다 계획에 정한 최대 파일 수와 요청 수를 넘지 않으며, budget을 넘는 파일은 애초에 후보에서 제외한다. 사전 파일군의 모든 파일이 budget을 넘으면 signature 없이 경로·확장자·크기만으로 묶고 그 사실을 기록한다.
...
- 장비별 최대 다운로드 파일 수
- 파일별 최대 바이트 (이 값을 넘는 파일은 다운로드하지 않고 건너뛴다)
- 전체 최대 바이트
- 최대 실행 시간
- 최대 동시 연결 수
- 초당 최대 요청 수
